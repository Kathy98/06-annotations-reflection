...
public interface ICNDBApi {
	...
	@GET("/jokes/random/{count}")
	Call<List<Joke>> getRandomJokes(@Path("count") int count, @Query("firstName") String firstName,
									@Query("lastName") String lastName);

	@GET("/jokes/random/{count}")
	Call<List<Joke>> getRandomJokes(@Path("count") int count, @Query("limitTo") String[] categories);
